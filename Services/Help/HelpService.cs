...
namespace Kaepora
{
    public class HelpService : ServiceBase
    {
        ...
        protected override Task PreEnable() => Task.CompletedTask;
        protected override Task PreDisable() => Task.CompletedTask;

        public static void RegisterCommandService(CommandService commandService)
        {
            _commandService = commandService;
        }

        private static readonly Regex _removeRedundantSpaces = new Regex(@"(?: {2,}|(?:[\r\n\t\f\v])+)", RegexOptions.Compiled);
        private static readonly Embed _noMatchFoundEmbed = new EmbedBuilder().WithColor(new Color(186, 44, 44)).WithDescription("No command or command group matched your search");
        public Embed HelpLookup(string query)
        {
            var searchString = _removeRedundantSpaces.Replace(query, " ");

            return formatModule(moduleSearch()) ?? formatCommand(commandSearch()) ?? _noMatchFoundEmbed;

            ModuleInfo moduleSearch() => _commandService.Modules.FirstOrDefault(m => m.Aliases.First() == searchString);
            CommandInfo commandSearch() => _commandService.Commands.FirstOrDefault(c => c.Aliases.First() == searchString);
